...
import plotly 
plotly.tools.set_credentials_file(username='cwong79', api_key='SjGQqzKTyFzUumvdbbLv')
...
def exercise02(new_observations):
    ...
    ir = pd.DataFrame(iris.data)
    ir.columns = iris.feature_names 
    features = ir.iloc[:, 1:5].values
    label = ir.iloc[:, 5].values
...
def exercise03(neighbors,split):
    ...
    knn_score = knn.score(y_test, pred)
...
def exercise05():
    ...
    # ------ Place code below here \/ \/ \/ ------
    Boston = pd.DataFrame(np.c_[boston['data'], boston['target']], columns= np.append(boston['feature_names'], ['target']))
    average_age = Boston["Age"].mean()
    average_medv = Boston["MEDV"].mean()
    medv_as_numpy_values = Boston["MEDV"].apply(np.mean)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise06():
    ...
    X_ptratio = Boston["PTRATIO"].to_numpy()
    # ------ Place code above here /\ /\ /\ ------
...
def exercise07():
    ...
    reg = lm()
    prediction_space = np.linspace(start=x.min(), stop=x.max()).reshape(-1,1)
